...
def load_attribute_mapping() -> Dict[str, Dict[str, str]]:
    ...
    try:
        if not config_file.exists():
            ...
            send_alert("config_loader", "ERROR", error_msg, {
        ...
        for attr_name, mapping in attribute_mapping.items():
            if not isinstance(mapping, dict):
                raise ValueError(f"Invalid mapping for attribute '{attr_name}': expected dict, got {type(mapping)}")
            
            if 'field' not in mapping or 'container' not in mapping:
                raise KeyError(f"Missing required keys ('field', 'container') for attribute '{attr_name}'")
